...
async function addProduct() {
  ...
  try {
    const result = await addProductService(data, userStore.token);
    if (result.code === 1) {
      ...
      <table class="w-full text-sm text-left text-gray-500 dark:text-gray-400">
        ...
        <tbody>
          <tr v-for="product in productStore.productList" :key="product.product_id" class="bg-white border-b dark:bg-gray-800 dark:border-gray-700">
            ...
            <td class="py-4 px-6 font-medium text-gray-900 whitespace-nowrap dark:text-white">{{ product.title }}</td>
